...
int dfs(int y, int x, vector<pair<int, int>> &vp, vector<vector<int>> &chk_tmp) 
{
    if (y >= 0 && y < n && x >= 0 && x < m && !chk_tmp[y][x] && v[y][x] != 1) 
    {
        ...
        {
            chk[y][x] = 1;
            return  dfs(y-1, x, vp, chk_tmp) + 
                    dfs(y+1, x, vp, chk_tmp) + 
                    dfs(y, x-1, vp, chk_tmp) + 
                    dfs(y, x+1, vp, chk_tmp) + 
                    1;
        }
...
int main() 
{
    ...
    {
        if (mp[{it->first.first.first, it->first.first.second}]) {
            it->second += mp[{it->first.first.first, it->first.first.second}];
        }

        if (mp[{it->first.second.first, it->first.second.second}]) {
            it->second += mp[{it->first.second.first, it->first.second.second}];
        }
